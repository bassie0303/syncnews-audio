...
- **TTSは言語で出し分け（日=Azure / 英=ElevenLabs）。OpenAI TTS不可** — OpenAI TTS は文字/語タイムスタンプを返さない。**日本語は Azure Speech**（`_azure_tts_ja()`、`ja-JP-NanamiNeural`）で原文の漢字をネイティブに読ませ、**SSMLブックマーク**(`<bookmark>`)の `audio_offset` から文単位タイムスタンプを得る。**英語は ElevenLabs** `/with-timestamps`（`tts_with_timestamps()`）で文字TSを文境界に集約。どちらも「文単位segment(start_ms/end_ms)」を作る点は共通。プロバイダや方式を差し替えると同期の根拠が消える。（→ *アーキテクチャ：タイムスタンプ取得*）
...
| `ELEVENLABS_API_KEY` | backend。**英語**TTS + 文字タイムスタンプ |
...
言語ごとに `synthesize_segments`（**日=Azure** `_azure_tts_ja` / **英=ElevenLabs** `tts_with_timestamps`）
で 音声＋文単位segment を生成 → Storage(`audio/{article_id}/{lang}.mp3`) と `tracks`/`segments` に保存。
重いTTSの前に ElevenLabs 残量を確認するクレジットガードあり（英語分のみ計上。日本語Azureは無料枠）。
...
### タイムスタンプ取得（言語別 / なぜこの構成か）
テキスト同期にはタイムスタンプが必須で OpenAI TTS単体では取れない。言語で出し分ける:
- **英語 = ElevenLabs**: `/text-to-speech/{voice}/with-timestamps`（`eleven_multilingual_v2`）で
  文字TSを取得 → `character_start/end_times_seconds` をms化 → `aggregate_to_sentences` で文境界に集約。
- **日本語 = Azure Speech**: ElevenLabsは漢字/数字/助数詞/拗音を誤読しがちで、カナ変換による
  回避も限界だったため移行。原文の漢字をそのまま SSML に入れ、各文の直前に `<bookmark mark='i'/>` を置く。
  `bookmark_reached` の `audio_offset`(100ns単位)が文iの開始時刻 → 文末＝次文開始（最後は総尺 `audio_duration`）。
  SDK(`azure-cognitiveservices-speech`)はブロッキングのため `asyncio.to_thread` で実行。Linux依存ライブラリは
  `backend/nixpacks.toml`（libasound2/libssl3）で導入。F0無料枠で運用。
